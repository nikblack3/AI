...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for p in self.particles:
                ...
                weighted_particles.append((p, emissionModel[distance]))
            weight_list = []
            for p in weighted_particles:
                weight_list.append(p[1])
            if not any(weight_list):
                ...
                for p in weighted_particles:
                    if not p[0] in dic:
                        ...
                        dic[p[0]] += p[1]
                for key in dic:
                    distribution[key] = dic[key] / total_weight
                new_particles = []
                for x in range(0, self.numParticles):
                    new_particles.append(util.sample(distribution))
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.particles:
            ...
            new_partile = util.sample(distribution)
            new_particles.append(new_partile)
        self.particles = new_particles
        "*** END YOUR CODE HERE ***"
...
class JointParticleFilter:
        ...
        while count < self.numParticles:
            for product in products:
                if count < self.numParticles:
                    ...
                    break #have to break, products may be too big
    ...
    def observeState(self, gameState):
        ...
        for p in self.particles:
            ...
            weighted_particles.append((p, weight))
        weight_list = []
        for p in weighted_particles:
            weight_list.append(p[1])
        if not any(weight_list):
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                new_ghost = util.sample(newPosDist)
                newParticle[i] = new_ghost
